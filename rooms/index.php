...
<body>
  ...
  <div>
    <div class="horizontalContainer">
      <h2 style="margin-right: 20px;">Rooms</h2>
    ...
    <table>
      <thead>
        <tr>
          <strong>
            ...
            echo "<strong>";
            echo "<th><a class='sort' href='?table=room&order=room_no&sort=$sort'>Room No.</a></th>";
            echo "<th><a class='sort' href='?table=room&order=room_type&sort=$sort'>Room Type</a></th>";
            echo "</strong>";
            ?>
            <th>Delete</th>
          </strong>
      ...
      <tbody>
          ...
          while ($row = $result->fetch_assoc()) {
            echo "
              <tr>
                ...
                <td>{$row['room_type']}</td>
                <td>
                <form action='delete.php?room_no={$row['room_no']}' method='POST'>
        ...
        if (isset($_GET['order']) && $result->num_rows == 0) {
          $order = $_GET['order'];
          $sql = "SELECT * FROM room ORDER BY $order $sort";
        }
        ?>
      </tbody>
    </table>
